...
data class SignalEntity(
    ...
    // --- AI fields, added alongside Signal's (see data/model/Signal.kt). Nullable/
    // defaulted so this is purely additive; DB version bumped to 2 with
    // fallbackToDestructiveMigration() in AppDatabase.kt since there is no prior
    // shipped install to preserve history for yet (see PATCH_NOTES.md). ---
    val decisionSource: String = "RULE_ENGINE",
    val aiRawScore: Double? = null,
    val aiCalibratedProbability: Double? = null,
    val aiEntryContext: String? = null,
...
fun Signal.toEntity(isBacktest: Boolean): SignalEntity = SignalEntity(
    ...
    aiCalibratedProbability = aiCalibratedProbability,

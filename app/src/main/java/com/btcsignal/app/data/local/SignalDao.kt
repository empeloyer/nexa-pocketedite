...
interface SignalDao {
    ...
    /** ALL still-unresolved Live signals, not just the newest one - used by
     *  LiveMonitoringService's stale-signal safety net (see resolveStaleActiveSignals):
     *  a single missed 1-minute kline can make CandleAggregator silently skip the
     *  FiveMinuteCandleClosed event for that candle (spec section 27 data-integrity
     *  choice), which otherwise leaves that specific signal stuck at ACTIVE forever
     *  even while every signal before and after it resolves normally. */
    @Query("SELECT * FROM signals WHERE isBacktest = 0 AND status = 'ACTIVE' ORDER BY signalTimestampMillis ASC")
    suspend fun getActiveLiveSignals(): List<SignalEntity>

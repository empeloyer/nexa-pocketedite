...
class BacktestEngine(
    ...
    private val aiModelManager: com.btcsignal.app.ai.ModelManager? = null,
    private val adaptiveLearning: com.btcsignal.app.ai.AdaptiveLearningEngine? = null,
    ...
    suspend fun run(
        ...
        if (aiModelManager?.currentInferenceEngine() == null) {
            aiModelManager?.initialize()
            aiModelManager?.activeCalibration?.let {
                adaptiveLearning?.seedFromCalibration(it, isAdverse = false)
                adaptiveLearning?.seedFromCalibration(it, isAdverse = true)
            }
        }
        // Same knob the Live screen reads on every checkpoint (LiveMonitoringService.
        // handleCheckpoint). Read once up front rather than per-checkpoint: a backtest
        // replay is a single run against a single set of user settings, so re-reading
        // per-candle would only add DataStore reads without changing the outcome, and
        // this keeps AI-off backtests (settingsRepository == null, e.g. in tests) an
        // explicit null instead of a default value that could silently mask this
        // parameter not being wired through at all.
        val userMinConfidence = settingsRepository?.settingsFlow?.first()?.aiConfidenceThreshold?.toDouble()
                ...
                when (event) {
                    is CandleEvent.CheckpointReached -> {
                        ...
                        val result = CoreSignalEngine.evaluateCheckpoint(
                            ...
                            userMinConfidence = userMinConfidence
                        )

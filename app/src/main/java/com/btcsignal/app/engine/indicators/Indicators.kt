...
object Indicators {
    ...
    // ---------- Added for the AI feature extractor (ai/AiFeatureExtractor.kt) ----------
    // Additive only - nothing above this line was changed, so every existing strategy's
    // indicator computation and existing tests are untouched.

    /** RSI series aligned to [closes] (same length, leading nulls until warm), Wilder-smoothed. */
    fun rsiSeries(closes: List<Double>, period: Int): List<Double?> {
        val n = closes.size
        val result = MutableList<Double?>(n) { null }
    ...
    /**
     * Stochastic RSI: a plain Stochastic %K applied to the RSI series itself instead of
     * price (standard definition). Not used by any of the 27 rule-based strategies -
     * added only as a causal feature for the AI model (see feature_schema.json).
     */
    fun stochRsi(closes: List<Double>, rsiPeriod: Int, stochPeriod: Int): Double? {
        val rsi = rsiSeries(closes, rsiPeriod)
        if (rsi.size < stochPeriod || rsi.takeLast(stochPeriod).any { it == null }) return null
        val window = rsi.takeLast(stochPeriod).map { it!! }
        val hi = window.max(); val lo = window.min()
        val latest = window.last()
        return if (hi == lo) 50.0 else (latest - lo) / (hi - lo) * 100.0
    }
}
